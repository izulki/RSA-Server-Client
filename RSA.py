...
# Function: Sign Message
    # Input: Private Exponent, Modulus, Message (Plaintext)
    # Output: Encrypted Message (Ciphertext)
def sign(d, n, userMsg_str):
    h = hashlib.md5(userMsg_str)
    #print("Signed Hash: " + h.hexdigest())
    userMsg_intArr = [(ord(c)) for c in h.hexdigest()] #list compression, convert each character to unicode (Decimal)
    #print(userMsg_intArr)
    userMsg_hexStr = "0x" + binascii.hexlify(bytearray(userMsg_intArr))
    #print(userMsg_hexStr)
    userMsg_bigInt = int(userMsg_hexStr, 16)
    #print(userMsg_bigInt)
    return pow(userMsg_bigInt, d, n)


# Function: Decrypt Message
    # Input: Private 5 Exponent, Modulus, Message (ciphertext)
    # Output: Decrypted Message (Plaintext)
def verify(n, e, signedtext, message):
    ptext = pow(int(signedtext, 10), e, n)
    #print(ptext)
    decode_hexStrRaw = hex(ptext) #string
    #print(decode_hexStrRaw)
    decode_hexStr = decode_hexStrRaw[2:-1]
    #print(decode_hexStr)
    gotHash = bytearray.fromhex(decode_hexStr)
    print(gotHash)
